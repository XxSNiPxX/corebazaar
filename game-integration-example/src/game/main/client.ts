  ...
  const client: RpgClientEngineHooks = {
      ...
      onConnected(engine: RpgClientEngine, socket: any) {
          ...
          socket.on("SendSignature", async (data: any) => {
              console.log(`Received "SendSignature" event from server:`, data);
              ...
              try {
                  ...
                  console.log(
                      typeof data.item_price,
                      ethers.parseEther(data.item_price.toString()),
                      Number(data.item_price),
                      "GOLD IASSS"
                  );

                  const tx = await contract_game.createTrade(
                      data.item_name,
                      data.quantity,
                      ethers.parseEther(data.item_price.toString()),
                      data.sellerid,
          ...
          socket.on("SendBuySignature", async (data: any) => {
              console.log(`Received "SendBuySignature" event from server:`, data);
              ...
              try {
                  ...
                  const tx = await contract_game.buyTrade(
                      ...
                      data.buyerid,
                      data.signature,
                      { value: ethers.parseEther(data.item_price.toString()) }
                  );
